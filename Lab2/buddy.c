...
// Check is bit set in position index
...
void bd_print() {
    for (int k = 0; k < nsizes; k++) {
        ...
        printf("  alloc:");
        bd_print_vector(bd_sizes[k].xorf, NBLK(k));
        if(k > 0) {
            printf("  split:");
...
void * bd_malloc(uint64 nbytes)
{
    ...
    bit_inverse(bd_sizes[k].xorf, blk_index(k, p));
    for(; k > fk; k--) {
        // split a block at size k and mark one half allocated at size k-1
        // and put the buddy on the free list at size k-1
        char *q = p + BLK_SIZE(k-1);   // p's buddy
        bit_set(bd_sizes[k].split, blk_index(k, p));
        bit_inverse(bd_sizes[k-1].xorf, blk_index(k-1, p));
...
void bd_free(void *p) {
    ...
    for (k = size(p); k < MAXSIZE; k++) {
        ...
        bit_inverse(bd_sizes[k].xorf, bi);  // free p at size k
        if (bit_get(bd_sizes[k].xorf, buddy)) {  // is buddy allocated?
...
void bd_mark(void *start, void *stop)
{
    ...
    for (int k = 0; k < nsizes; k++) {
        ...
        for(; bi < bj; bi++) {
            ...
            bit_inverse(bd_sizes[k].xorf, bi);
        }
    }
}
