...
// adds new process to the head of the linked list of processes
void push_front(struct proc* el){

    el->prev = &proclist; //previous is a list head
    el->next = proclist.next; // next is a ex-first element of list
    
    proclist.next->prev = el; //previous for ex-first is new element
    proclist.next = el; //el is first element of*/
}

// remove process from list
void pop(struct proc* el){
...
// to prevent race with process being moved&wait_lock
// to a different CPU.
int
cpuid() {
    int id = r_tp();
...
allocproc(void) {
    ...
    //Allocate a kernel stask (was in proc_mapstacks)
...
int dump2(int pid, int register_num, uint64 *return_value){
	...
	int mid = my_proc->pid;
  	...
  	for (struct proc* it = proclist.next; it != &proclist; it = it->next){
 		acquire(&proclist.lock);
    		if(it->pid == pid){
...
reparent(struct proc *p) {
    for (struct proc *pp = proclist.next; pp != &proclist; pp = pp->next) {
        if (pp->parent == p) {
            pp->parent = initproc;
            wakeup(initproc);
        }
...
sleep(void *chan, struct spinlock *lk) {
    ...
    if(lk != &proclist.lock){
        acquire(&proclist.lock);  //DOC: sleeplock1
        release(lk);
    }
